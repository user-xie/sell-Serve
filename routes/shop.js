  ...
  conn.query(sql, (err, data) => {
    ...
    if (data.length) {
      ...
      for (let key in shopdata) {
        if (key === "date") {
			if (shopdata[key].indexOf("[") != -1)
				shopdata[key] = JSON.parse(shopdata[key]);
			else 
				shopdata[key] = "数据库数据格式你改错了!!!正确格式为['1970-01-01 08:00:00','1970-01-01 22:30:00']!!!字符串引号为双引号!快去修复!!"
        }
		...
		if (key === "supports") {
			if (shopdata[key].indexOf("[") != -1)
				...
				shopdata[key] = "数据库数据格式你改错了!!!正确格式为['单人精彩套餐','果汁全场8折','在线支付满28减5']!!!字符串引号为双引号!快去修复"
        }
		
		if (key === "pics") {
			if (shopdata[key].indexOf("[") != -1){
        shopdata[key] = JSON.parse(shopdata[key]);
				shopdata.pics = shopdata.pics.map(img => "/upload/shop/" + img)
			}
			else 
				shopdata[key] = "数据库数据格式你改错了!!!正确格式为['1590480883773.webp','1590480886706.webp','1590480889717.webp','1590482092433.webp']!!!字符串引号为双引号!快去修复"
      }
      }
    }
    // console.log(data[0])

    shopdata.avatar = "/upload/shop/" + shopdata.avatar
    


    res.send({ data: shopdata });
  });
});
